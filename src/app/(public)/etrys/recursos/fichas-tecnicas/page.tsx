...
export default function RSTFichasTecnicasPage() {
  ...
  useEffect(() => {
    fetch("/api/recursos?division=RST&activo=true")
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#0099ce] via-[#007ba8] to-[#00269b] text-white py-12">
              ...
              {filtered.map((recurso, index) => (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-14 h-14 bg-[#0099ce]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                      <FileText size={28} className="text-[#0099ce]" />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="bg-[#0099ce]/10 text-[#0099ce] text-xs font-medium px-2 py-0.5 rounded uppercase">
                          {recurso.tipo}
                        </span>
