...
export default function EICFichasTecnicasPage() {
  const [recursos, setRecursos] = useState<RecursoAPI[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");

  useEffect(() => {
    fetch("/api/recursos?division=EIC&activo=true")
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero */}
      <section className="bg-linear-to-br from-[#009e49] via-[#007d3a] to-[#00269b] text-white py-12">
        <div className="container-eminsa">
          <nav className="flex items-center gap-2 text-sm text-white/60 mb-6">
            ...
            <Link href="/eic" className="hover:text-white transition-colors">EIC</Link>
            <ChevronRight size={14} />
            <Link href="/eic/recursos" className="hover:text-white transition-colors">
              Recursos
            </Link>
            <ChevronRight size={14} />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/70">Documentación técnica de productos EIC</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container-eminsa">
          <div className="relative max-w-lg">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#009e49] focus:border-transparent"
            />
          </div>
        </div>
      </section>
        ...
        <div className="container-eminsa">
          {loading ? (
            <div className="flex justify-center py-16">
              <div className="w-8 h-8 border-4 border-[#009e49] border-t-transparent rounded-full animate-spin" />
            </div>
          ) : filtered.length === 0 ? (
              ...
              {filtered.map((recurso, index) => (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-14 h-14 bg-[#009e49]/10 rounded-xl flex items-center justify-center shrink-0">
                      <FileText size={28} className="text-[#009e49]" />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="bg-[#009e49]/10 text-[#009e49] text-xs font-medium px-2 py-0.5 rounded uppercase">
                          {recurso.tipo}
                        </span>
      ...
      <section className="py-12 bg-white">
        <div className="container-eminsa text-center">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 bg-[#009e49] hover:bg-[#007d3a] text-white px-8 py-3 rounded-xl font-semibold transition-colors"
          >
